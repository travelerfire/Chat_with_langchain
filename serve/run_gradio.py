...
class Model_center():
    ...
    def qa_chain_self_answer(self, question: str, chat_history: list = [], model: str = "zhipuai", embedding="zhipuai", temperature: float = 0.0, top_k: int = 4, file_path: str = DEFAULT_DB_PATH, persist_path: str = DEFAULT_PERSIST_PATH):
        """
        调用不带历史记录的问答链进行回答
        """
        if question == None or len(question) < 1:
